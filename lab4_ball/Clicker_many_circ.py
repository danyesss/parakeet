...
l = Label(root, bg='white', fg='black',width=600, text="Let's start! Just click.")
...
[x, y, r, clr, dx, dy, circ] = [Empty_x,Empty_y,Empty_r,Empty_clr, Empty_dx, Empty_dy, Empty_circ]
#
def newest_ball():
    ...
    for i in range(n_circ):
        ...
        r[i] = rnd(30,50)
        clr[i]= choice(colors)        
        circ[i] = canv.create_oval(x[i]-r[i],y[i]-r[i],x[i]+r[i],y[i]+r[i],fill = clr[i], width=0)
...
def update():
    global x, y, r, dx, dy
    for i in range(n_circ):
        while abs(x[i]+dx[i]-400)>=400-r[i] or abs(y[i]+dy[i]-300)>=300-r[i]:
            dx[i] = (rnd(500)-250)/50
            dy[i] = (rnd(500)-250)/50	          
        x[i] += dx[i]
        y[i] += dy[i]    
